...
use sophia::{api::term::Term, term::{ArcTerm, GenericLiteral}};
...
pub fn call_function(function: &Function, arguments: Vec<EvalResult>) -> Option<EvalResult> {
    match function {
        Str => {
            let [arg] = &arguments[..] else { unreachable!() };
            str(arg)
        }
        Lang => todo("Lang"),
        ...
        Regex => todo("Regex"),
        Triple => {
            let [s, p, o] = &arguments[..] else { unreachable!() };
            triple(s, p ,o)
        }
...
pub fn str(er: &EvalResult) -> Option<EvalResult> {
    Some(match er.as_term() {
        ...
        ArcTerm::Triple(t) => {
            ...
            for term in t.iter() {
                sophia::turtle::serializer::nt::write_term(&mut buf, term.borrow_term()).ok()?;
                buf.push(b' ');
            }
            buf.extend(b">>");
